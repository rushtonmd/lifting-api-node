...
*/
  ...
  module.exports = {
    ...
    findOne: function(name, query, callback) {
      module.exports.find(name, query, 1, callback);
    },
    ...
    setValue: function(name, username, id, updateQuery, callback) {
      console.log(username + " " + id + " " + updateQuery);
    ...
    findAndRemove: function(name, query, callback) {
      db.collection(name).remove(query, {w:1}, callback);
      //db.collection('userprofiles').remove({username: 'testuser'}, {w:1}, callback);
    }
  }
})();